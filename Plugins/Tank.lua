...
function OnLTick500ms()
	...
	if (myself ~= nil) and (myself:GetClass() == 139) then
		if (TankStatus) then 
			...
			ChainHydra = MySkills:FindById(10016)
			...
			if (myself:IsAlikeDeath() == false) and (GetDistanceVector(FarmPoint,myself:GetLocation()) < 5000) then	
				if (HateAuraSkill ~= nil) and (HateAuraSkill:IsSkillAvailable()) and (AllowedMass < CurrentTime) and CheckSorrounding() then
					...
					if (AOESkill3 ~= nil) and (AOESkill3:IsSkillAvailable()) and (AOESkill3 ~= nil) and (AOESkill3:IsSkillAvailable())  then
						UseSkillRaw(10012, false, false);
						elseif (AOESkill2 ~= nil) and (AOESkill2:IsSkillAvailable()) then  
						UseSkillRaw(10014, false, false);
						AllowedMass = CurrentTime + 1000;
						else
